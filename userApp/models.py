...
class User(AbstractUser, BaseModel):
    ...
    def create_verify_code(self, verify_type):
        code = str(random.randint(1010, 9990))
...
class UserConfirmation(BaseModel):
    ...
    user = models.ForeignKey('userApp.User', models.CASCADE, related_name='verif_codes')
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            if self.verify_type == VIA_EMAIL:
                self.expiration_time = datetime.datetime.now() + datetime.timedelta(minutes=EMAIL_EXPIRE)
            else:
                self.expiration_time = datetime.datetime.now() + datetime.timedelta(minutes=PHONE_EXPIRE)
